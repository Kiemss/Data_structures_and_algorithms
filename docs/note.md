...
15. 数组指针
    ...
    // 一维数组的正确指针声明
    int *ptr1 = arr;                    // 指向元素
    int (*ptr1_alt)[3] = &arr;          // 指向整个数组
    
    // 二维数组的正确指针声明  
    int (*ptr2)[3] = _arr;              // 指向一行（最常用）
    int (*ptr3)[3][3] = &_arr;          // 指向整个二维数组
    
    // 错误的尝试
    // int *ptr4 = _arr;                // 错误！_arr最多退化为 int (*ptr)[n]
    // int *ptr5[3] = arr;              // 错误！ptr5是指针数组，不是指针,也就是要用int (*)[n]
    
    //⭐总结：用数组指针能正确指向整个数组，否则就要考虑数组指针退化为下一维
    //由于[]的结合优先级比*和&高，所以int *ptr[n]是指针数组，int (*ptr)[n]才是数组指针
    ```
