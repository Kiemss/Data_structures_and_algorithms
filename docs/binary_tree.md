...
* **特点**：一种带权路径长度最短的二叉树，用于数据压缩编码
* **核心思想**：出现频率高的字符使用短编码，出现频率低的字符使用长编码

## 构建过程

1. **统计频率**：统计待编码字符集中每个字符的出现频率
2. **创建节点**：为每个字符创建一个节点，权重为其频率
3. **构建森林**：所有节点构成一个森林（每个节点是一棵树）
4. **合并节点**：
   - 每次选择权重最小的两棵树进行合并
   - 创建新节点作为这两棵树的父节点，权重为子节点权重之和
   - 将新树放回森林中
5. **重复合并**：直到森林中只剩下一棵树，即为哈夫曼树
...
- **构建哈夫曼树**：O(n log n)，使用优先队列（堆）实现
- **编码**：O(n)
- **解码**：O(n)

## 适用场景

* **文本压缩**：对英文文本、程序代码等有较好压缩效果
* **图像压缩**：JPEG格式中的熵编码阶段
* **通信传输**：减少数据传输量
* **配置文件**：对重复出现的关键字进行压缩
...
压缩文件结构：`文件头信息 + 哈夫曼编码表 + 压缩数据`

**具体原因**

1. **编码表存储开销**：需要存储所有字符的编码映射关系
2. **固定开销**：文件头、校验信息等元数据
3. **字节对齐填充**：压缩后的位流需要补足到整字节

**数据对比**：
- 原始小文件：可能仅10-50字节
- 编码表+文件头：通常需要30-100字节
- 最终结果：压缩后文件 > 原始文件
